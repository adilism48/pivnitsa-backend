...
### US-35 Отчёт по броням и выручке (владелец)

Генерирует и отдает отчёт для скачивания в формате Excel (`.xlsx`) для владельца за указанный период.
Содержит сводные метрики (общее количество броней, сумма успешных предоплат, количество отмен) и детализированный список транзакций.

```http
GET /api/v1/admin/bookings/report?startDate=...&endDate=...
```
